...
CONFIG_SCHEMA = {
    ...
    "properties": {
        "server": {
            "type": "object",
            "required": ["ipaddress", "port", "secretkey", "private_key_password", "identity", "password"],
            "properties": {
        ...
        "eap_methods": {
            "type": "object",
            "required": ["tls", "ttls_tls", "peap_tls", "peap_mschapv2", "ttls_md5", "ttls_mschapv2", "eap_fast"],
            "properties": {
...
def load_config(config_path: str) -> Dict:
    ...
    if not os.path.exists(config_path):
        logger.error(f"Config file {config_path} not found. Create it using 'eaptestor --generate-config'.")
...
def generate_config_template(output_path: str):
    ...
    template = {
        ...
        "eap_methods": {
            "tls": {"enabled": false, "config": {"certificate_file": "", "private_key_file": "", "private_key_passwd": "", "ca_cert": ""}},
            "ttls_tls": {"enabled": false, "config": {"certificate_file": "", "private_key_file": "", "private_key_passwd": "", "ca_cert": ""}},
            "peap_tls": {"enabled": false, "config": {"certificate_file": "", "private_key_file": "", "private_key_passwd": "", "ca_cert": ""}},
